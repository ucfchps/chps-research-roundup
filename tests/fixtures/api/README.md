...
| `pubmed-efetch-norte-and-collision.xml` | Real `efetch.fcgi?db=pubmed&rettype=abstract&retmode=xml` for 6 PMIDs: 3 real Norte G. (CHPS faculty) papers + 3 from a live `Chen S[Author]` esearch (a global name-collision surname, used elsewhere to demonstrate `retmax=250` over-broad queries) | Confirmed via direct inspection, not assumed: UCF affiliation text present and matching (pmid 42268385); UCF affiliation present but on author 4 of 6, not the first author (pmid 42246438 — Norte's own paper, proving "check every author" isn't optional); clearly non-UCF affiliation, Chinese institutions (pmids 42561073/42561035/42561017 — the Chen S. collisions); and a real author (Palmer, pmid 42561073) with **two** `<AffiliationInfo>` blocks on one `<Author>` — dual-institution authors are a real shape, not a hypothetical |
| `pubmed-efetch-old-no-affiliation.xml` | Real `efetch.fcgi` for 3 arbitrary low-numbered (1970s) PMIDs | Affiliation coded on **zero** authors across all 3 records (`<Author>` present with `<LastName>`/`<ForeName>`/`<Initials>`, no `<AffiliationInfo>` child at all) — PubMed's affiliation-capture coverage is itself incomplete, independent of any UCF-matching question; confirms "absent entirely" is a real production shape, not an edge case invented for test coverage |
